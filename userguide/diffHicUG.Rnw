...
<<>>=
results.b <- data.frame(anchor1=as.data.frame(boxed$anchor1)[,1:3],
...
best.mid.a1 <- (results.b$best.anchor1.start[chosen]+results.b$best.anchor1.end[chosen])/2
best.mid.a2 <- (results.b$best.anchor2.start[chosen]+results.b$best.anchor2.end[chosen])/2
...
image(as.matrix(temp), zlim=color.range)
