...
nested <- data.frame(anchor1=as.data.frame(a1x.n)[,1:3],
    anchor2=as.data.frame(a2x.n)[,1:3], inside[, c("logFC", "F")])
head(nested)
...
write.table(results.n[o.s,], file="nested_best.tsv", sep="\t", quote=FALSE, row.names=FALSE)
@
