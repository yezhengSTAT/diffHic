...
This results in the formation of very large clusters that are difficult to interpret.
Some protection is provided by setting \code{upper} to limit the maximum dimensions of each cluster.
This will cause excessively large clusters to be broken up, albeit in a somewhat arbitrary manner.
...
<<>>=
clustered.small$anchor1
clustered.small$anchor2
@

The FDR across clusters is not the same as the FDR across the individual bin pairs.
The former is more useful as results are usually interpreted in terms of clusters, where each cluster corresponds to a single underlying interaction.
However, the FDR across bin pairs is easier to compute by applying the BH method to all bin pairs.
Treating the FDR across bin pairs as that across clusters is usually inappropriate and will result in loss of FDR control \citep{lun2014denovo}.
This section will deal with some strategies for controlling the cluster-level FDR.

\subsection{Clustering based on significant bin pairs}
Ideally, clustering is done in a manner that is independent of the test statistics, e.g., using the abundance.
This means that established methods can be used to control the FDR across clusters (see Section~\ref{sec:independentcluster}).
However, the interaction space is often dominated by high-abundance structural features like domains and compartments. 
Attempting to cluster bin pairs here may result in excessive chaining, yielding very large clusters for which the coordinates are mostly uninformative with respect to DIs.
...
length(clustered.sig$anchor1)
@

The \code{clusterPairs} function is used internally to perform clustering on significant bin pairs. 
No limits are placed on the maximum dimensions, as the sparsity of bin pairs should avoid chaining effects.
Additional sophistication can be obtained by setting \code{fc.col="logFC"}.
This will cluster bin pairs separately if they are changing in opposite directions.

The above \code{diClusters} call will attempt to control the cluster-level FDR below 5\%.
...
The estimated cluster-level FDR can be obtained in the \code{FDR} field of the output.
Admittedly, this procedure is not statistically rigorous and will yield biased estimates for small numbers of clusters.
However, this may be a necessary price to pay for interpretable results in high-density areas of the interaction space.
...
This will report the number of bin pairs in each cluster, the number changing in each direction, the bin pair with the lowest $p$-value, etc.
...
Results can be easily saved to file as shown below.
Here, coordinates of the bounding box for each cluster are saved along with some useful cluster statistics.
...
Resorting makes it easier to parse the final table, as the most interesting differential interactions are placed at the top.

<<>>=
results.d <- data.frame(anchor1=as.data.frame(clustered.sig$anchor1)[,1:3], 
    anchor2=as.data.frame(clustered.sig$anchor2)[,1:3], tabcom[,1:4], 
    logFC=tabbest$logFC, FDR=clustered.sig$FDR)
o.d <- order(results.d$PValue)
write.table(results.d[o.d,], file="clustered.tsv", sep="\t", quote=FALSE, row.names=FALSE)
@

\subsection{Independent clustering of adjacent bin pairs}
\label{sec:independentcluster}
The basic idea of independent clustering is that it is done independently of the test statistics.
...
All bin pairs overlapping a pre-defined area can be defined as a single cluster.
Otherwise, filtering on the average abundance will implicitly restrict the clustering to high-abundance bin pairs.
This is less problematic when the interaction space is sparse, such that the clusters will be small enough to be interpretable.
To demonstrate, a quick-and-dirty differential analysis is performed for the 100 kbp bin pairs.
...
A combined $p$-value is computed for each cluster from the $p$-values of its bin pairs, using the method described by \cite{simes1986}.
This represents the evidence against the global null hypothesis, i.e., that none of the constituent bin pairs are significant.
Cluster-level FDR control is maintained by applying the BH method on the combined $p$-values for all clusters.
Significant clusters can then be defined at a certain FDR threshold, e.g., 5\%.
This is the statistically rigorous alternative to the \textit{post-hoc} approach in \code{diClusters}.
...
Results can then be saved to file as previously shown.
Here, the $p$-value and FDR from \code{combineTests} have valid statistical interpretations and are retained in the result table.
Metrics from \code{getBestTest} can also be added if so desired.

<<>>=
results.i <- data.frame(anchor1=as.data.frame(clustered.small$anchor1)[,1:3], 
    anchor2=as.data.frame(clustered.small$anchor2)[,1:3], tabcluster)
...
For large bin sizes, users may wish to interpret each bin pair on its own without any clustering.
In such cases, the BH method can be applied directly to the $p$-values for the individual bin pairs. 
The FDR here refers to the proportion of detected bin pairs that are false positives, rather than the proportion of detected clusters. 
...
These can be saved to file as described.
Here, the bin pair-level results are used as no clustering is performed. 

<<>>=
results.r <- data.frame(anchor1=as.data.frame(anchors(data, type="first"))[,1:3],
    anchor2=as.data.frame(anchors(data, type="second"))[,1:3], result$table, FDR=adj.p)
...
Both \code{diClusters} and \code{clusterPairs} accept multiple \code{InteractionSet} objects to cluster results from different bin sizes.

<<>>=
merged.data <- list(larger=data, smaller=smaller.data)
...
This acts as a form of independent clustering where all nested bin pairs are defined as a single cluster within the parent.
...
Statistics for each cluster can be computed using the \code{consolidatePairs} function.
For independent clustering, this uses a weighted version of Simes' method to ensure each analysis contributes equally to the significance of each cluster \citep{benjamini1997multiple}.
For each bin pair, the weight of the $p$-value is inversely proportional to the number of bin pairs of the same size in the same cluster. 
This ensures that the combined $p$-value calculation for each cluster is not dominated by smaller, more numerous bin pairs.
(Similarly, \code{diClusters} uses a frequency-weighted version of the FDR to ensure an equal contribution from each bin size during clustering.)
...
<<>>=
results.n <- data.frame(anchor1=as.data.frame(boxed$anchor1)[,1:3],
    anchor2=as.data.frame(boxed$anchor2)[,1:3], cons)
o.n <- order(results.n$PValue)
write.table(results.n[o.n,], file="nested.tsv", sep="\t", quote=FALSE, row.names=FALSE)
@
...
nested <- data.frame(anchor1=as.data.frame(a1x.n)[,2:3],
    anchor2=as.data.frame(a2x.n)[,2:3], inside[, c("logFC", "F")])
head(nested)
expanded <- rep(NA, nrow(results.n)) # As some parents do not have nested elements.
expanded[as.integer(rownames(inside))] <- seq_len(nrow(inside))
results.n <- data.frame(results.n, nest=nested[expanded,])
write.table(results.n[o.n,], file="nested_best.tsv", sep="\t", quote=FALSE, row.names=FALSE)
@
