...
This section will describe some strategies for controlling the cluster-level FDR.

\subsection{Clustering based on significant bin pairs}
Ideally, clustering would be done in a manner that is independent of the differential test statistics, e.g., based on bin pairs remaining after filtering on the average abundance.
...
In such high-density areas, too many bin pairs may be retained after filtering such that clustering would result in excessive chaining.
This would yield very large clusters for which the coordinates provide no information on DIs.
...
Results can be easily saved to file for later examination, as shown below.
Here, the coordinates of the bounding box for each cluster are saved along with some of the useful cluster statistics.
...
This represents the evidence against the global null hypothesis, i.e., that none of the constituent bin pairs are significant in the cluster.
...
Results are saved to file below.
Here, the $p$-value and FDR from \code{combineTests} are retained as they have valid statistical interpretations.
Metrics from \code{getBestTest} can also be added.
...
For large bins, users may wish to interpret each bin pair on its own without any clustering.
In such cases, the BH method can be applied directly to the bin pair $p$-values.
...
These can be saved to file along with the bin pair-based results.
...
chosen <- o.n[1]
chosen.a1 <- boxed$anchor1[chosen]
chosen.a2 <- boxed$anchor2[chosen]
...
chosen <- o.n[3]
example <- boxed$anchor2[chosen]
end(example) <- end(boxed$anchor1[chosen])
