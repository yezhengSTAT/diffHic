...
The larger index refers to the ``first'' anchor fragment whereas the smaller is the second.
...
\label{sec:strorient}
The strand orientation for a read pair refers to the combination of strands for the two reads.
These are stored as flags where setting \code{0x1} or \code{0x2} means that the reads on the first or second fragment, respectively, are mapped on the reverse strand.
...
For full compatibility, users should include the alignment positions and lengths as \code{anchorX.pos} and \code{anchorX.len} for both reads (i.e., \code{X} set to 1 or 2).
...
This generates a \code{InteractionSet} object that contains the relevant count and coordinate information.
...
Each row of the returned \code{RangedSummarizedExperiment} contains the marginal read counts for a single genomic bin.
